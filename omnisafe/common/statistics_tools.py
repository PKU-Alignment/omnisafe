...
from omnisafe.utils.tools import assert_with_exit, recursive_dict2json, update_dic
...
class StatisticsTools:
    ...
    def draw_graph(self, parameter: str, values: list = None, compare_num: int = None):
        """Draw graph.
        ...
        assert_with_exit(
            ...
            f'parameter `{parameter}` has only one value `{parameter_values[0]}`, no need to compare',
        )
        ...
        for graph_dict in graph_paths:
            ...
            save_name = recursive_dict2json(decompressed_img_name_cfgs)
    ...
    def make_config_groups(self, parameter, parameter_values: list, values: list, compare_num: int):
        ...
        for img_name_cfgs, graph in graph_groups:
            ...
            for path_dict in graph:
                exp_name = recursive_dict2json(path_dict)
