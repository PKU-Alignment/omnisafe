...
class SACLag(SAC, Lagrange):  # pylint: disable=too-many-instance-attributes
    """Continuous control with deep reinforcement learning (SAC) Algorithm.
    ...
    def compute_loss_pi(self, data: dict):
        ...
        Returns:
            torch.Tensor
        """
        action, logp_a = self.actor_critic.actor.predict(data['obs'], deterministic=True, need_log_prob=True)
        loss_pi = self.actor_critic.critic(data['obs'], action)[0] - self.alpha*logp_a
        penalty = self.lambda_range_projection(self.lagrangian_multiplier).item()
        loss_pi -= (
            self.lagrangian_multiplier * self.actor_critic.cost_critic(data['obs'], action)
        )
        loss_pi /= (1 + penalty)
        pi_info = {}
    ...
    def compute_loss_c(self, data):
        ...
        with torch.no_grad():
            act_targ, logp_a_next = self.ac_targ.actor.predict(obs_next, deterministic=True, need_log_prob=True)
