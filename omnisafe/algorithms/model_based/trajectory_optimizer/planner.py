...
class CEMPlanner:
    ...
    def _state_action_repeat(self, state, action, num_particles):
        """
        Repeat the state for num_repeat * action.shape[0] times and action for num_repeat times
        """
        assert action.shape == (self._num_samples, self._action_shape), "Input action dimension should be equal to (self._num_samples, self._action_shape)"
        assert state.shape[0] == 1 , "state dimension one should be 1"
        states = state.repeat(num_particles * action.shape[1], 1)
        actions = actions.unsqueeze(1).repeat(num_particles, 1)
        return states, actions


    def _compute_actions_return(self, actions, traj, returns_key='rewards', values=None):
        """
    ...
    def output_action(self,state):
        ...
        for iter in range(self._num_iterations):
            ...
            states_repeat, actions_repeat = self._state_action_repeat(state, actions, self._num_particles)
            # [num_particles * num_samples, state_shape], [horizon, num_particles * num_samples, action_shape]
            ...
            returns = self._compute_actions_return(actions, traj['rewards'])
            # [num_sample, 1]
