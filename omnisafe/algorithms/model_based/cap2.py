...
import os
from omnisafe.algorithms.model_based.planner.planner import CCEPlanner
...
from omnisafe.algorithms.model_based.models import VirtualEnv
...
class CAP(BaseAlgo):
    ...
    def _init_model(self) -> None:
        ...
        self._dynamics = EnsembleDynamicsModel(
            ...
            reward_size=1,
            cost_size=1,
            network_size=self._cfgs.dynamics_cfgs.network_size,
            elite_size= self._cfgs.dynamics_cfgs.elite_size,
            hidden_size = self._cfgs.dynamics_cfgs.hidden_size,
            use_decay = self._cfgs.dynamics_cfgs.use_decay,
        )

        self.virtual_env = VirtualEnv('CAP', self._dynamics, self._env_id, self._device)

        self._planner = CCEPlanner(
            self._dynamics_state_space.shape[0],
            self._device,
            self._env,
            self.virtual_env,
            7,
    ...
    def update_dynamics_model(self, current_step):
        ...
        self._logger.store(
            **{
                ...
                'Loss/DynamicsValMseLoss': val_mse_losses.mean().item(),
            }
        )

    def _select_action(self, time_step, state, env):
        """action selection"""
        action = self._planner.get_action(np.array(state.squeeze(0).cpu().detach().numpy()))
        action = torch.tensor(action).to(self._device).unsqueeze(0)
