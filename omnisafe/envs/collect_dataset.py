...
class DrawCircle(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        self.action_low = np.repeat(self.action_space.low, self.parallel_num).reshape(self.parallel_num, -1)
        self.action_high = np.repeat(self.action_space.high, self.parallel_num).reshape(self.parallel_num, -1)
        self.obs_low = np.repeat(self.observation_space.low, self.parallel_num).reshape(self.parallel_num, -1)
        self.obs_high = np.repeat(self.observation_space.high, self.parallel_num).reshape(self.parallel_num, -1)
    ...
    def reset(
        ...
        *,
        ...
        self.trajectory = np.zeros([self.parallel_num, self._max_episode_step, self.observation_space.shape[0]])
        return random_reset, {}
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, bool, dict]:
        ...
        if self.env_config['control_type'] == 'velocity':
            ...
            self.item_pos = np.clip(self.item_pos,
                                    self.obs_low[:, :2],
                                    self.obs_high[:, :2])
    ...
    def step_cost(self):
        ...
        cost[np.where(
            (item_distance < self.env_config['constraints'][1]) & (
                item_distance > self.env_config['constraints'][0]))] = 0
...
def collect_expert_trajectory(traj_num=1000, type='circle', save_path='./data'):
        ...
        if type == 'circle':
            action = np.stack(
                [-radiums * freq * np.sin(freq * time + theta),
                 radiums * freq * np.cos(freq * time + theta)], axis=1)
        elif type == 'rect':
            edge = radiums / 2 ** 0.5
            ...
            action = np.stack(
                [freqL * np.cos(current_theta),
                 freqL * np.sin(current_theta)], axis=1)
    ...
    for i in range(traj_num):
        ...
        theta[np.where((obs[:, 0] < 0) & (obs[:, 1] > 0))] = np.pi - theta[np.where((obs[:, 0] < 0) & (obs[:, 1] > 0))]
        theta[np.where((obs[:, 0] < 0) & (obs[:, 1] < 0))] = np.pi - theta[np.where((obs[:, 0] < 0) & (obs[:, 1] < 0))]
        ...
        while not done:
            ...
            action = action_sample(step=step, radiums=radiums, theta=theta, freq=freq, type=type,
