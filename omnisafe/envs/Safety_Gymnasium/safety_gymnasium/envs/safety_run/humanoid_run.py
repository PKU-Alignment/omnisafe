...
class SafetyHumanoidRunEnv(MujocoEnv, utils.EzPickle):
    """
    ## Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.goal_pos = np.array([0, -1e-3])
        self._reward_factor = 60.0  # Reward for forward locomotion tests (vel in x direction)
        ...
        MujocoEnv.__init__(
            self, "humanoid_run.xml", 5, observation_space=observation_space, **kwargs
        )
    ...
    def step(self, action):
        ...
        # Hyperparameters
        x_pos = xy_position_after[0]
        x_lim = 2.5

        # Get reward
        reward = 0.0
        potential = -np.linalg.norm(xy_position_after - self.goal_pos) * self._reward_factor
        reward += potential - self.old_potential
        self.old_potential = potential

        # Get cost
        cost = float(np.abs(x_pos) > x_lim)
    ...
    def reset_model(self):
        ...
        self.old_potential = -np.linalg.norm(self.get_body_com("torso")[:2].copy() - self.goal_pos) * self._reward_factor
