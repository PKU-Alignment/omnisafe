...
class SafetyAntCircleEnv(CircleMujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        # Get reward
        reward = (np.sqrt(x_velocity**2 + y_velocity**2)) / (1 + np.abs(xy_distance - radius)) + 0.5*self.healthy_reward
        reward -= self.control_cost(action)
        # Get cost
