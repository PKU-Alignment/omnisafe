...
Optimization (PCPO)** is an iterative method for optimizing policy in a **two-stage
process**: the first stage performs a local reward improvement update,
while the second stage reconciles any constraint violation by
projecting the policy back onto the constraint set.

PCPO is an improvement work done on the basis of **CPO**
(:doc:`../SafeRL/CPO`). It provides a lower bound on reward
...
PCPO based on two different metrics: :math:`L2` norm and KL
divergence.
...
.. note:: 
   ...
   that you read this article after reading the CPO tutorial (:doc:`./CPO`) we wrote.
...
where :math:`\Pi_{\theta}\subseteq\Pi` denotes the set of
...
optimizing over :math:`\Pi_{\theta}`, PCPO optimizes over
:math:`\Pi_{\theta}\cap\Pi_{C}`. Next, we will introduce you to how
...
PCPO performs policy update in **two stages**. The first stage is :bdg-ref-info-line:`Reward Improvement Stage<two stage update>`
which maximizes reward using a trust region optimization method without constraints.
This might result in a new intermediate policy that does not satisfy the
constraints. The second stage named :bdg-ref-info-line:`Projection Stage<two stage update>` reconciles the constraint violation (if
...
.. tab-set::
    ...
    .. tab-item:: Stage 1
      ...
      .. card:: 
         ...
         :math:`\pi_{k+\frac12}` to be within a :math:`\delta`-neighbourhood of :math:`\pi_{k}`:
         ...
         This update rule with the trust region is called **TRPO** (sees in :doc:`../BaseRL/TRPO`). It constraints the policy
...
The :bdg-ref-info-line:`Projection Stage<two stage update>` ensures that the constraint-satisfying policy
:math:`\pi_{k+1}` is close to :math:`\pi_{k+\frac12}`. The :bdg-ref-info-line:`Reward Improvement Stage<two stage update>`
ensures that the agent's updates are in the direction
of maximizing rewards so as not to violate the step size of distance
measure :math:`D`. :bdg-ref-info-line:`Projection Stage<two stage update>`  causes the agent to update in the
...
Implementation of Two Satge Update
===================================

For a large neural network policy with hundreds of thousands of
parameters, directly solving for the PCPO update in :math:`(2)` and :math:`(3)` is impractical
...
.. tab-set::
    ...
    .. tab-item:: Implementation of Stage 1
      ...
      .. card:: 
         ...
         semi-definite), :math:`b\doteq J^{C}(\pi_k)-d` is the constraint
    ...
    .. tab-item:: Implementation of Stage 2
      ...
      .. card:: 
         ...
         projection is defined in the parameter space, :math:`L2` norm projection is
         ...
         where :math:`\boldsymbol{L}=\boldsymbol{I}` for :math:`L2` norm projection, and
         :math:`\boldsymbol{L}=\boldsymbol{H}` for KL divergence projection.
         +++
...
.. tab-set::
   ...
   .. tab-item:: Answer II
      ...
      .. card:: 
         ...
         Answer
         ^^^
         By sequentially projecting onto each of the sets, the
         update in :math:`(7)` can be
...
The update rule in :math:`(7)` shows that
the difference between PCPO with KL divergence and :math:`L2` norm projections
...
.. card:: 
   ...
   PCPO with :math:`L2`  norm projection converges to a stationary point either
...
:bdg-info-line:`Theorem 3` shows that
...
of KL divergence and :math:`L2` norm projections.
   ...
   * Run Agent from custom terminal config
   ...
   .. tab-set::
     ...
     .. tab-item:: Terminal config style
            ...
            PCPO simply using ``train_on_policy.py``, with arguments about PCPO and enviroments
            does the training. For example, to run PCPO in SafetyPointGoal1-v0 , with
...
-  `Projection-Based Constrained Policy Optimization
   <https://arxiv.org/pdf/2010.03152.pdf>`__
-  `Trust Region Policy
...
infeasible ( constraint-violating), we first prove two lemma os the KL divergence between
...
.. tab-set::
    ...
    .. tab-item:: Proof of Lemma 1
        ...
        .. card:: 
            ...
            Proof of Lemma 1
            ^^^
            By the Bregman divergence projection inequality,
            :math:`\pi_{k}` being in the constraint set, and :math:`\pi_{k+1}`
            being the projection of the :math:`\pi_{k+\frac{1}{2}}` onto the
    ...
    .. tab-item:: Proof of Lemma 2
      ...
      .. card:: 
         ...
         Proof of Lemma 2
         ^^^
         We define the sublevel set of cost constraint function for
         ...
         :math:`L^{\pi_k}`, and :math:`\pi_{k+\frac{1}{2}}` is projected onto
         the constraint set:
         ...
         For these three polices
         :math:`\pi_k, \pi_{k+1},` and :math:`\pi_{k+1}^l`, with
         ...
         :math:`L^{\pi_k}`, and :bdg-info-line:`Lemma 1`.
...
.. card:: 
   ...
   :math:`\boldsymbol{L}=\boldsymbol{I}` if using the :math:`L2`  norm
   ...
   .. dropdown:: Proof of Analytical Solution to PCPO (Click here)
      ...
      For the first problem, since :math:`\boldsymbol{H}` is the
      ...
      which also satisfies :math:`(22)` and
      :math:`(24)`. Hence by
      :math:`(18)` and
      :math:`(25)`, we have
...
.. card:: 
   ...
   .. dropdown:: Proof of Lemma 3 (Click here)
      ...
      Since the right hand side of :math:`(26)` can be made
...
.. card:: 
   ...
   .. dropdown:: Proof of Theorem 3 (Click here)
      ...
      The proof of the theorem is based on working in a Hilbert
      space and the non-expansive property of the projection. We first
      prove stationary points for PCPO with the KL divergence and :math:`L2` norm
      ...
      :math:`(29)` boils down
      to :math:`g\in-a,` and for the :math:`L2` norm projection
      ...
      .. note::
         ...
         the objective value for PCPO with the :math:`L2` norm projection improves, the
         right hand side of :math:`(26)` needs to be
