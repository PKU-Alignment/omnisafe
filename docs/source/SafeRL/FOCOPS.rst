...
Instead of solving the Problem :math:`(1)` ~ :math:`(3)` directly, FOCOPS uses a **two-stage** approach summarized below:
...
.. card::
    ...
    Two-stage Policy Update
    ^^^^^^^^^^^^^^^^^^^^^^^
    -  Given policy :math:`\pi_{\theta_k}`, find an optimal update policy :math:`\pi^*` by solving the optimization problem from Problem :math:`(1)` in the non-parameterized policy space.
...
In the first stage, FOCOPS rewrites Problem :math:`(1)` ~ :math:`(3)` as below:
...
These problems are only slightly different from Problem :math:`(1)` ~ :math:`(3)`, that is, the parameter of interest is now the nonparameterized Policy :math:`\pi` and not the policy parameter :math:`\theta`.
...
.. card::
    ...
    If :math:`\pi_{\theta_k}` is a feasible solution, the optimal policy for Problem :math:`(4)` ~ :math:`(6)` takes the form
    ...
    where :math:`Z_{\lambda,\nu}(s)` is the partition function which ensures Problem :math:`(7)` is a valid probability distribution, :math:`\lambda` and :math:`\nu` are solutions to the optimization problem:
...
We will refer to the optimal solution to Problem :math:`(4)` ~ :math:`(6)` as the *optimal update policy*.
Suppose you need help understanding the meaning of the above Equation.
In that case, you can first think that FOCOPS finally solves Problem :math:`(4)` ~ :math:`(6)` by solving Problem :math:`(7)`and Problem :math:`(8)`.
...
Note that Equation :math:`(10)` can be estimated by sampling from the trajectories generated by Policy :math:`\pi_{\theta_k}` so Policy can be trained using stochastic gradients.
...
We use that data and Equation :math:`(8)` first to estimate :math:`\lambda` and :math:`\nu`.
We then draw a minibatch from the data to estimate :math:`\nabla_\theta \mathcal{L}(\theta)` given in :bdg-info-line:`Corollary 1`.
After taking a gradient step using Equation:math:`(10)`, we draw another minibatch and repeat the process.
...
.. hint::
    ...
    Solving Problem :math:`(8)` is computationally impractical for large state or action spaces as it requires calculating the partition function :math:`Z_{\lambda,\nu}(s)`, which often involves evaluating a high-dimensional integral or sum.
...
With strong duality, the optimal :math:`\lambda^*` and :math:`\nu^*` minimizes the dual function :math:`(8)` which then be denoted as :math:`L(\pi^*,\lambda,\nu)`.
...
The last term in the gradient expression in Equation :math:`(12)` cannot be evaluated since we do not have access to :math:`\pi^*`.
...
Using the update rule :math:`(13)`, FOCOPS performs one update step on :math:`\nu` before updating the Policy parameters :math:`\theta`.
A per-state acceptance indicator function :math:`I\left(s_j\right)^n:=\mathbf{1}_{D_{\mathrm{KL}}\left(\pi_\theta \| \pi_{\theta_k}\right)\left[s_j\right] \leq \delta}` is added to :math:`(10)`, in order better to enforce the accuracy for the first-order purposed method.
...
.. tab-set::
    ...
    .. tab-item:: Analysis of :math:`\lambda`
        ...
        .. card::
            ...
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^
            In Equation :math:`(7)`, note that as :math:`\lambda \rightarrow 0`, :math:`\pi^*` approaches a greedy policy;
    ...
    .. tab-item:: Analysis of :math:`\nu`
        ...
        .. card::
            ...
            Analysis of :math:`\nu`
            ^^^^^^^^^^^^^^^^^^^^^^^
            We recall that in Equation :math:`(7)`,
            ...
            Hence in this regard, the update rule in :math:`(13)` is intuitive,
...
.. card::
    ...
    Since :math:`J^{C}(\pi_{\theta_k})` is a constant w.r.t :math:`\pi`, constraint :math:`(5)` is linear.
    Constraint :math:`(6)` can be rewritten as :math:`\sum_s d^{\pi_{\theta_k}}(s) D_{\mathrm{KL}}\left(\pi \| \pi_{\theta_k}\right)[s] \leq \delta`.
    The KL divergence is convex w.r.t its first argument.
    Hence Constraint :math:`(5)`, a linear combination of convex functions, is also convex.
    Since :math:`\pi_{\theta_k}` satisfies Constraint :math:`(6)` also satisfies Constraint :math:`(5)`, therefore Slater's constraint qualification holds, and strong duality holds.

.. dropdown:: Proof of Theorem 1 (Click here)
    :color: info
    :class-body: sd-border-{3}

    Based on :bdg-info-line:`Lemma 1` the optimal value of the Problem :math:`(4)` ~ :math:`(6)` :math:`p^*` can be solved by solving the corresponding dual problem.
    Let
    ...
    Note that if :math:`\pi^*`, :math:`\lambda^*`, :math:`\nu^*` are optimal for Problem\ :math:`(15)`, :math:`\pi^*` is also optimal for Problem :math:`(4)` ~ :math:`(6)` because of the strong duality.

    Consider the inner maximization problem in Problem :math:`(15)`.
    ...
    Which is equivalent to the inner maximization problem in :math:`(15)`.
    ...
    Setting Equation :math:`(18)` to zero and rearranging the term, we obtain:
    ...
    We find that the optimal solution :math:`\pi^*` to Equation :math:`(16)` takes the form
    ...
    Plugging the result back to Equation :math:`(15)`, we obtain:
...
.. tab-set::
   ...
   .. tab-item:: Proof of Corollary 2
        ...
        .. card::
            ...
            Plugging :math:`(24)`  into the last equality in :math:`(22)`  gives us:
            ...
            Combining :math:`(25)`  with the derivatives of the affine term give us the final desired result.
